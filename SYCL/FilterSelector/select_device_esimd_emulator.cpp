...
// RUN: env SYCL_DEVICE_FILTER=esimd_emulator:gpu,host %t.out
//
...
// available.
//
// REQUIRES: esimd_emulator,gpu,host
...
int main() {
...

  {
    ...
    assert(name.find("ESIMD_EMULATOR") != string::npos);
    cout << "ESIMD_EMULATOR GPU Device is found: " << boolalpha << d.is_gpu()
         << std::endl;
  }
  {
    gpu_selector gs;
